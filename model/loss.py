...
class VariationalE(nn.Module):
    def __init__(self,cls_num_list,reweight_epoch,alpha,tau,max_m=0.5):
    ...
    def forward(self,epoch,x,y,trivial_utility=True,extra_info=None):
        # compute utility
        if not trivial_utility and epoch>self.reweight_epoch:
            batch_cls_idx = torch.stack([self.cls_idx]*len(y))
            relative_class_idx = batch_cls_idx-y.unsqueeze(-1)
            utility = -torch.ones_like(batch_cls_idx)*(relative_class_idx<0)
